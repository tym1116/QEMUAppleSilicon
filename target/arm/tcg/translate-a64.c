...
static void do_fp_st(DisasContext *s, int srcidx, TCGv_i64 tcg_addr, int size)
{
    ...
    if (size < 4) {
        ...
        tcg_gen_qemu_st_i64(be ? tmplo : tmphi, tcg_hiaddr, get_mem_index(s),
                            mop);
    }
}
...
static void do_fp_ld(DisasContext *s, int destidx, TCGv_i64 tcg_addr, int size)
{
    ...
    if (size < 4) {
        ...
        tcg_gen_qemu_ld_i64(be ? tmphi : tmplo, tcg_addr, get_mem_index(s),
                            mop | (s->align_mem ? MO_ALIGN_16 : 0));
        tcg_gen_addi_i64(tcg_hiaddr, tcg_addr, 8);
        tcg_gen_qemu_ld_i64(be ? tmplo : tmphi, tcg_hiaddr, get_mem_index(s),
                            mop);
    }
...
static void disas_comp_b_imm(DisasContext *s, uint32_t insn)
{
    ...
    tcg_gen_brcondi_i64(op ? TCG_COND_NE : TCG_COND_EQ, tcg_cmp, 0,
                        match.label);
    gen_goto_tb(s, 0, 4);
...
static void disas_ld_lit(DisasContext *s, uint32_t insn)
{
    ...
    if (is_vector) {
        ...
        do_gpr_ld(s, tcg_rt, clean_addr, size + is_signed * MO_SIGN, false,
...
static void disas_ldst_pair(DisasContext *s, uint32_t insn)
{
    ...
    if (is_vector) {
        ...
        if (is_load) {
             ...
             */
            do_gpr_ld(s, tmp, clean_addr, size + is_signed * MO_SIGN, false,
                      false, 0, false, false);
            tcg_gen_addi_i64(clean_addr, clean_addr, 1 << size);
            do_gpr_ld(s, tcg_rt2, clean_addr, size + is_signed * MO_SIGN, false,
                      false, 0, false, false);
...
static void disas_ldst_ldapr_stlr(DisasContext *s, uint32_t insn)
{
    ...
    if (is_store) {
         ...
         */
        do_gpr_ld(s, cpu_reg(s, rt), clean_addr, mop, extend, true, rt, iss_sf,
                  true);
        tcg_gen_mb(TCG_MO_ALL | TCG_BAR_LDAQ);
    }
}
...
static void disas_ldst_multiple_struct(DisasContext *s, uint32_t insn)
{
     ...
     */
    clean_addr =
        gen_mte_checkN(s, tcg_rn, is_store, is_postidx || rn != 31, total);
...
static void disas_ldst_single_struct(DisasContext *s, uint32_t insn)
{
    ...
    clean_addr =
        gen_mte_checkN(s, tcg_rn, !is_load, is_postidx || rn != 31, total);
